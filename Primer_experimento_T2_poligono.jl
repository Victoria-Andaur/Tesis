...
function resolver_y_guardar(divisions_x::Int, divisions_y::Int,N::Int, filas, cols, x_val1, y_val1, J2::Int, K::Int, E2::Array{Float64}, R2::Array{Float64}, 
    ...
    graficar_solucion(Turbinas_solucion,Receptores,"Imágenes/2. Exp2 Turbinas T2/Mapa Turbinas T2_divx"*string(divisions_x)*"divy_"*string(divisions_y)*"N_"*string(N)*".svg",J2)
"""
    # Paso 7: Guardar resultados
    guardar_resultados("Experimento 2/Turbinas T2_divx"*string(divisions_x)*"divy_"*string(divisions_y)*"N_"*string(N)*".txt",N,round(Energia,digits=2),round(elapsed_time,digits=3),R2,Receptores,Turbinas_solucion)
"""
    return Turbinas_solucion, Energia,Mz
end
...
Turbinas_solucion,Energia_resultado,Mz=resolver_y_guardar(
    divisions_x, divisions_y,N, filas, cols, 
     x_val1, y_val1, J2, K, E2, R2, Receptores)
