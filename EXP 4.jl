...
function resolver_y_guardar(divisions_x::Int, divisions_y::Int,N::Int, filas, cols, 
    ...
    graficar_solucion(Turbinas_solucion,Receptores,"Imágenes/4. Exp4 Turbinas T1/Mapa Turbinas T1_divx"*string(divisions_x)*"divy_"*string(divisions_y)*"N_"*string(N)*".svg",J)

    # Paso 7: Guardar resultados
    guardar_resultados("Experimento 4/Turbinas T1_divx"*string(divisions_x)*"divy_"*string(divisions_y)*"N_"*string(N)*".txt",N,round(Energia,digits=2),round(elapsed_time,digits=3),R,Receptores,Turbinas_solucion)
...
Turbinas_solucion_1_Exp4,Energia_1_exp4,Mz_exp4,tiempo_1_exp4=resolver_y_guardar(
    divisions_x, divisions_y,N, filas,cols, x_val1, y_val1, J1, K, E1, R1, Receptores);

Energia_resultado_exp4=Dict()
tiempo_calculo_exp4=Dict()
Turbinas_solucion_exp4=Dict()
Ruido_exp4=Dict()
Energia_resultado_exp4[1]=Energia_1_exp4
Turbinas_solucion_exp4[1]=Turbinas_solucion_1_Exp4
tiempo_calculo_exp4[1]=tiempo_1_exp4
ruido_lista_Exp4=[]
for k in 1:7
    push!(ruido_lista_Exp4, round(10*log10(sum(10^(R1[Turbinas_solucion_1_Exp4."Index_y"[i],Turbinas_solucion_1_Exp4."Index_x"[i],Turbinas_solucion_1_Exp4."MODO"[i]+1,k]/10) for i in 1:length(Turbinas_solucion_1_Exp4."Index_x"))), digits=2))
end
Ruido_exp4[1]=ruido_lista_Exp4
...
Indices=[]
for aero in eachrow(Turbinas_solucion_1_Exp4)
...
function iteraciones_exp4(Turbinas_solucion,Poligono,J,K,R,N,filas,cols,x_val1,y_val1,E,iter)
    Indices=[]
    ...
    ruido_lista_Exp4=[]
    for k in 1:7
        push!(ruido_lista_Exp4, round(10*log10(sum(10^(R[Turbinas_solucion_iter."Index_y"[i],Turbinas_solucion_iter."Index_x"[i],Turbinas_solucion_iter."MODO"[i]+1,k]/10) for i in 1:length(Turbinas_solucion_iter."Index_x"))), digits=2))
    end
    graficar_solucion(Turbinas_solucion_iter,Receptores,"Imágenes/4. Exp4 Turbinas T2 3D 7D/Mapa Turbinas T2_3D_7D_iter_divx"*string(divisions_x)*"divy_"*string(divisions_y)*"N_"*string(N)*"_iter"*string(iter)*".svg",J)
    return Turbinas_solucion_iter,Energia, elapsed_time,ruido_lista_Exp4,size(Turbinas_solucion_iter)[1]
end



turbinas_agregadas_exp4=Dict()
turbinas_agregadas_exp4[1]=size(Turbinas_solucion_exp4[1])[1]
for i in 2:1001
    Turbinas_solucion_exp4[i],Energia_resultado_exp4[i],tiempo_calculo_exp4[i],Ruido_exp4[i],turbinas_agregadas_exp4[i]=iteraciones_exp4(Turbinas_solucion_exp4[i-1],Poligono,J1,K,R1,N,x_step,y_step,filas,cols,x_val1,y_val1,E1,i)
    if Turbinas_solucion_exp4[i-1]==Turbinas_solucion_exp4[i]
        println(i)
        break
    end
end
Guardar_resultado_50_20_exp4_T1=DataFrame("Iteración"=>Int[],"Turbinas agregadas"=>Int[],"Energia producida [MW]"=>Float64[],"Tiempo de cálculo [s]"=>Float64[],"Ruido receptor 1 [dB]"=>Float64[],"Ruido receptor 2 [dB]"=>Float64[],"Ruido receptor 3 [dB]"=>Float64[],"Ruido receptor 4 [dB]"=>Float64[],"Ruido receptor 5 [dB]"=>Float64[],"Ruido receptor 6 [dB]"=>Float64[],"Ruido receptor 7 [dB]"=>Float64[])
for i in 1:1001
    push!(Guardar_resultado_50_20_exp4_T1,(i,turbinas_agregadas_exp4[i],Energia_resultado_exp4[i],tiempo_calculo_exp4[i],Ruido_exp4[i]...))
end
CSV.write("Experimento 4/Resultados 1001 50x20 Exp4 T1.csv",Guardar_resultado_50_20_exp4_T1)
CSV.write("Experimento 4/Turbinas 50x20 Exp4 T1 1000.csv",Turbinas_solucion_exp4[1000])
CSV.write("Experimento 4/Turbinas 50x20 Exp4 T1 1001.csv",Turbinas_solucion_exp4[1001])
CSV.write("Experimento 4/Turbinas 50x20 Exp4 T1 1.csv",Turbinas_solucion_exp4[1])
CSV.write("Experimento 4/Turbinas 50x20 Exp4 T1 500.csv",Turbinas_solucion_exp4[500])
CSV.write("Experimento 4/Turbinas 50x20 Exp4 T1 700.csv",Turbinas_solucion_exp4[700])
CSV.write("Experimento 4/Turbinas 50x20 Exp4 T1 2.csv",Turbinas_solucion_exp4[2])


tur_sol_5020_T2_1001,energia_1001, t_iter_1001,ruido_lista_Exp4_1001_T2,t_n_T2=iteraciones_exp4(tur_sol_5020_T2,Poligono,J2,K,R2,N,filas,cols,x_val1,y_val1,E2,1001)

tur_sol_5020_T2[!,:MODO] = convert.(Int64,tur_sol_5020_T2[!,:MODO]);
tur_sol_5020_T2[!,:NUM] = convert.(Int64,tur_sol_5020_T2[!,:NUM]);
tur_sol_5020_T2[!,:Index_x] = convert.(Int64,tur_sol_5020_T2[!,:Index_x]);
tur_sol_5020_T2[!,:Index_y] = convert.(Int64,tur_sol_5020_T2[!,:Index_y]);