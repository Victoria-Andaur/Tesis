...
function func_ruido_turbina_octavas_T2(j)
    indice=findall(z->z==Int(round(12)), Modos_T2[j]."V[m/s]")
    return [Modos_T2[j]."31Hz"[indice[1]],Modos_T2[j]."63Hz"[indice[1]],Modos_T2[j]."125Hz"[indice[1]],Modos_T2[j]."250Hz"[indice[1]],Modos_T2[j]."500Hz"[indice[1]],Modos_T2[j]."1kHz"[indice[1]],Modos_T2[j]."2kHz"[indice[1]],Modos_T2[j]."4kHz"[indice[1]],Modos_T2[j]."8kHz"[indice[1]]]
end
...
function ruido_octavas_T2(x,y,j,k)
    dist=distancia_turbina_1(x,y,k)
    if dist!=0
        #ate_div=(20*log10(dist)+11)
        ate_div=(10*log10(4*pi*dist^2))
        ate_atm=func_ruido_turbina_octavas_T2(j)-atenuacion_atmosferica(dist)
        return max(suma_ruido(ate_atm)-ate_div,0)
    else
        return func_ruido_turbina_1_T2(x,y,j)
    end
end

R=zeros(Float64,filas,cols, J2, K)
for x in 1:filas
    for y in 1:cols
        for j in 1:J2
            for k in 1:K
                R[x,y,j,k]=ruido_octavas_T2(x,y,j,k)
            end
        end
    end
end
...
for j in 1:J2
    for k in 1:K
        CSV.write("Mapa Ruido T2 octavas/MapaRuido_T2_Modo"*string(j-1)*"_receptor "*string(k)*"_octavas.csv", DataFrame(R[:,:,j,k],:auto))
    end
end
