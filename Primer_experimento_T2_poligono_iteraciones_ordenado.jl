...
for j in 1:J2
    E2[:,:,j]=readdlm("Mapa Energía T2 V2/MapaEnergia T2 Modo "*string(j-1)*".csv",',',header=false);
end
    ...
    for j in 1:J2
        for iy in 1:divisions_y
            for ix in 1:divisions_x
                objective_sum += E2[round(Int, ix * x_step - x_step + 1), round(Int, iy * y_step - y_step + 1), j] * binary_decision_vars[ix, iy, j];
            end
        end
    end
...
function Agregar_restriccion_ruido(R2, divisions_y, divisions_x, K, binary_decision_vars, modelo2)
    for k in 1:K
        @constraint(modelo2,(sum(sum(sum(10^(R2[round(Int,ix*x_step-x_step+1),round(Int,iy*y_step-y_step+1),j,k]/10)*binary_decision_vars[ix,iy,j] for ix in 1:divisions_x) for iy in 1:divisions_y) for j in 1:J2))<=10^4)
    end
end
...
function graficar_solucion(Turbinas,Receptores,Nombre,J2)
    ...
    plot!([Turbinas."X [m]";x_dummy],[Turbinas."Y [m]";y_dummy],seriestype=:scatter,marker_z = [Turbinas."MODO";modo_dummy],colorbar_title = "Modo de ruido",  color = cgrad(paleta, J2, categorical=true), label="Turbinas")
...
function mostrar_ruido_receptores(ruido,Recep,Turb)
    for k in 1:length(Recep[:,1])
        ...
        print(k)
        print(" es ")
        print(round(10*log10(sum(10^(ruido[Turb."Index_x"[i],Turb."Index_y"[i],Turb."MODO"[i]+1,k]/10) for i in 1:length(Turb."Index_x"))),digits=2))
        print("\n")
    end
end
...
function guardar_resultados(filename::String, N::Int, Energia::Float64, tiempo::Float64,ruido,Recep,Turb)
    ruido_lista=[]
    for k in 1:length(Recep[:,1])
        push!(ruido_lista, round(10*log10(sum(10^(ruido[Turb."Index_x"[i],Turb."Index_y"[i],Turb."MODO"[i]+1,k]/10) for i in 1:length(Turb."Index_x"))), digits=2))
    end
...
function resolver_y_guardar(divisions_x::Int, divisions_y::Int,N::Int, filas, cols, 
    x_val0, x_val1, y_val0, y_val1, J2::Int, K::Int, E2::Array{Float64}, R2::Array{Float64}, 
    Receptores::DataFrame)

    # Paso 1: Calcular pasos en X e Y
    x_step=floor(filas/divisions_x);
    y_step=floor(cols/divisions_y);
    ...
    for ix in 1:divisions_x
        for iy in 1:divisions_y
            @constraint(modelo2,sum(binary_decision_vars[ix,iy,:])<=Int(en_poligono(round(Int,ix*x_step-x_step+1),round(Int,cols-(iy*y_step-y_step+1)),Poligono)))
        end
    end
    ...
    for ix in 1:divisions_x
        for iy in 1:divisions_y
            for j in 1:J2
                if Mz[ix, iy, j] != 0
                    push!(Turbinas_solucion, ((((ix - 1) * x_step + 1) * (x_val1) / filas),( ((iy - 1) * y_step + 1) * (y_val1) / cols),cont, j-1, (ix - 1) * x_step + 1, (iy - 1) * y_step + 1))
                    cont += 1
                end
            end
        end
    ...
    guardar_resultados("Experimento 3/Turbinas T3_divx"*string(divisions_x)*"divy_"*string(divisions_y)*"N_"*string(N)*".txt",N,round(Energia,digits=2),round(elapsed_time,digits=3),R2,Receptores,Turbinas_solucion)
    return Turbinas_solucion
end

divisions_x=50;
divisions_y=50;
x_step=div(filas, divisions_x);
y_step=div(cols, divisions_y);
N=30;

Turbinas_solucion=resolver_y_guardar(
    divisions_x, divisions_y,N, filas, cols, 
    x_val0, x_val1, y_val0, y_val1, J2, K, E2, R2, Receptores);
...
function funcion_objetivo_iter(J2, E2, binary_decision_vars, modelo2,Indices)
    ...
    for (index,(ix,iy)) in enumerate(Indices)
        for j in 2:J2
            objective_sum += E2[ix,iy,j] * (binary_decision_vars[index, index, j]-binary_decision_vars[index, index, j-1]);
        end
        objective_sum += E2[ix,iy,1]*binary_decision_vars[index, index, 1]
    end
    @objective(modelo2, Max, objective_sum);
end
...
Función que agrega la restricción de que se pueden seleccionar a lo más N turbinas.
"""
function Agregar_restriccion_a_lo_más_n_turbinas(binary_decision_vars, modelo,N)
    suma=0
    for valor in 1:num_var
        for j in 1:(J2-1)
            suma+=binary_decision_vars[valor,valor,j]-binary_decision_vars[valor,valor,j+1]
        end
    end
    @constraint(modelo,suma+N>=0)
end
"""
    Restriccion_ruido(R2, divisions_y, divisions_x, K, binary_decision_vars, modelo2)
...
function Agregar_restriccion_ruido_iter(R2, Indices, K, binary_decision_vars, modelo_iter,J2)
    for k in 1:K
        ...
        for (index, (ix,iy)) in enumerate(Indices)
            for j in 2:J2
                suma_ruido+=10^(R2[ix,iy,j,k]/10)*(binary_decision_vars[index,index,j]-binary_decision_vars[index,index,j-1])
            end
            suma_ruido+=10^(R2[ix,iy,1,k]/10)*binary_decision_vars[index,index,1]
        end
    ...
    for (valor,(ix,iy)) in enumerate(Indices)
        for (valor2, (ix2,iy2)) in enumerate(Indices)
            if (ix2,iy2) in valid_variations(ix,iy)
                if valor2!=valor
                    @constraint(modelo_iter,sum(binary_decision_vars[valor,valor,j+1]-binary_decision_vars[valor,valor,j]+binary_decision_vars[valor2,valor2,j+1]-binary_decision_vars[valor2,valor2,j] for j in 1:(J2-1))<=1)
                end
            end
        end
    end
...
function Agregar_restriccion_distancia_entre_turbinas_iter(binary_decision_vars,modelo,x_step,y_step,Indices)
    for (valor,(ix,iy)) in enumerate(Indices)
        for (valor2, (ix2,iy2)) in enumerate(Indices)
            if valor2!=valor
                if abs(ix-ix2)<=13 && abs((iy-iy2))<=30
                    @constraint(modelo,binary_decision_vars[valor,valor,10]+binary_decision_vars[valor2,valor2,10]<=1)
                end
            end
        end
    end
...
function iteraciones(Turbinas_solucion,Poligono,J2,K,R2,N,x_step,y_step,filas,cols,x_val1,y_val1,E2,iter)
    Indices=[]
    for aero in eachrow(Turbinas_solucion)
        index_x = aero["Index_x"]
        index_y = aero["Index_y"]
    ...
    set_optimizer_attribute(modelo_iter, "TimeLimit", 60) 
    ...
    Agregar_restriccion_a_lo_más_n_turbinas(binary_decision_vars_iter, modelo_iter, 5);
    Agregar_restriccion_ruido_iter(R2, Indices, K, binary_decision_vars_iter, modelo_iter,J2)
    Agregar_restriccion_distancia_entre_turbinas_iter(binary_decision_vars_iter,modelo_iter,x_step,y_step,Indices)
    for (valor,(ix,iy)) in enumerate(Indices)
        @constraint(modelo_iter,binary_decision_vars_iter[valor,valor,10]<=Int(en_poligono(ix,cols-iy,Poligono)))
    end
    ...
    for (index,(ix,iy)) in enumerate(Indices)
        for j in 3:J2
            if (Mx[index, index, j]- Mx[index, index, j-1])!= 0 
                if (Mx[index, index, j-1]- Mx[index, index, j-2])== 0
                    push!(Turbinas_solucion_iter, ((ix) * (x_val1)/filas,(iy) * y_val1/cols,cont, j, ix, iy))
                    cont += 1
                end
            end
        end
        if Mx[index, index, 1]!= 0
            push!(Turbinas_solucion_iter, ((ix) * (x_val1)/filas,(iy) * y_val1/cols,cont, 0, ix, iy))
            cont += 1
        end
        if Mx[index, index, 2]!= 0 && Mx[index, index, 1]== 0
            push!(Turbinas_solucion_iter, ((ix) * (x_val1)/filas,(iy) * y_val1/cols,cont, 1, ix, iy))
            cont += 1
        end
    end
    graficar_solucion(Turbinas_solucion_iter,Receptores,"Imágenes/4. Exp4 Turbinas T2 3D 7D/Mapa Turbinas T2_3D_7D_iter_divx"*string(divisions_x)*"divy_"*string(divisions_y)*"N_"*string(N)*"_iter"*string(iter)*".svg",J2)
    return Turbinas_solucion_iter,Energia
end
